...
- Antigravity IDE: The editor application (VS Code fork with AI agent).
- Antigravity's native AI: The AI coding agent inside the IDE.
- Gemini CLI: A command-line tool (`gemini`) that runs the same
  underlying agent engine outside the IDE.

This adapter provides two execution mechanisms:
  1. Native Python SDK (`google.antigravity`) — Direct async Agent lifecycle,
     typed events, hooks, workspace sandbox, and multi-turn sessions.
  2. Gemini CLI subprocess (`gemini.cmd` / `gemini`) — Headless stream-json execution.

ALL Antigravity-specific logic is contained in this file.
The AI Manager never imports this class directly.
"""
...
# Check if native google.antigravity SDK is importable
...
def _map_event_type(raw_type: str) -> EventType:
    """Map event type strings to our enum."""
    mapping = {
        "init": EventType.INIT,
...
def _load_env_file():
    """Helper to load .env file if present in workspace."""
    for path in [Path(".env"), Path("../.env"), Path("e:/Work/automation/.env")]:
        if path.exists():
            try:
...
class _TaskSession:
    """Internal state for a single running task."""
    ...
    def __init__(
        ...
        self.file_snapshot_before: dict[str, float] = {}  # path -> mtime
    ...
    def _detect_file_changes(self) -> list[FileChange]:
        """Compare current file state to pre-task snapshot."""
    ...
    def snapshot_files(self):
        """Take a snapshot of files in the working directory before the task."""
        work_dir = Path(self.request.working_dir)
        if not work_dir.exists():
...
class AntigravityAdapter(IDEAgentAdapter):
    ...
    Features:
      - Native Python SDK support (google.antigravity)
      - Headless CLI subprocess support (gemini.cmd / gemini)
      - Streaming progress & structured events
      - File change diff detection
      - Process lifecycle & emergency abort
    """
    ...
    async def start_task(self, request: TaskRequest) -> str:
        """Start a new task via Antigravity SDK or Gemini CLI."""
        ...
        # If API key is present and SDK is available, prefer SDK
        if _SDK_AVAILABLE and self._api_key:
            ...
            try:
                # Add GEMINI_API_KEY to subprocess env if available
                env = os.environ.copy()
                if self._api_key:
    ...
    async def is_available(self) -> bool:
        """Check if SDK or CLI is available."""
        if _SDK_AVAILABLE:
            return True
        try:
    ...
    async def get_adapter_info(self) -> dict:
        """Return adapter metadata and verified capabilities."""
        ...
        return {
            ...
            "distinction": {
                "antigravity_ide": "The VS Code-forked IDE application",
                "antigravity_ai_agent": "The AI coding agent runtime inside Antigravity",
                "gemini_cli": "Command-line interface to the shared agent runtime",
                "python_sdk": "google-antigravity Python SDK for direct in-process control",
            },
        }
    ...
    async def _run_sdk_task(self, session: _TaskSession):
        """Execute task using the native google.antigravity SDK."""
        try:
    ...
    async def _read_cli_output(self, session: _TaskSession):
        """Read and parse stream-json output from Gemini CLI process."""
        ...
        try:
            while True:
                ...
                # Check if CLI is asking for interactive browser login
    ...
    def _parse_cli_event(self, raw: dict) -> TaskEvent:
        """Parse raw event dictionary from CLI."""
    ...
    async def _kill_session(self, session: _TaskSession):
        """Terminate process or cancel task."""
